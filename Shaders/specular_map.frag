...
// He probado a aumentar el brillo de la especular (*1.5) porque no notaba mucho cambio
// Así, si lo noto, pero no sé si está bien
...
vec3 calculateDirectionalLight(int i, vec3 lightDir, vec3 normal, vec3 viewDir) {
    ...
    if (lam > 0.0) {
        ...
        vec3 specColor = texture2D(specmap, f_texCoord).rgb * 1.5;
        i_spec = spec * theLights[i].specular * specColor;
    }
...
vec3 calculateLocalLight(int i, vec3 l, vec3 vertPos, vec3 normal, vec3 viewDir) {
	...
	float attenuation = 1.0 / (theLights[i].attenuation.x + theLights[i].attenuation.y * length(theLights[i].position.xyz - vertPos) + theLights[i].attenuation.z * pow(length(theLights[i].position.xyz - vertPos), 2.0));

    float NoL = lambert(normal, l);
    if (NoL > 0.0) {
        i_diff = theLights[i].diffuse * theMaterial.diffuse * attenuation;

        vec3 reflectDir = reflect(-l, normal);
        float spec = pow(lambert(viewDir, reflectDir), theMaterial.shininess);
        vec3 specColor = texture2D(specmap, f_texCoord).rgb * 1.5;
        i_spec = spec * theLights[i].specular * specColor * attenuation;
    }
